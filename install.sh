...
# Date :  2021-02-01
# Notes:  GS_TL_Env for CentOS/RedHat 7+ Debian 8+ and Ubuntu 14+
...
check_os()
{
  ...
  # Check OS Version
  if [ ${CentOS_ver} -lt 7 >/dev/null 2>&1 ] || [ ${Debian_ver} -lt 9 >/dev/null 2>&1 ] || [ ${Ubuntu_ver} -lt 18 >/dev/null 2>&1 ]; then
    echo "${CFAILURE}此系统版本不支持, 请安装 CentOS 7+,Debian 8+,Ubuntu 16+ ${CEND}"
    kill -9 $$
  fi
